...
export class BombermanGame {
    ...
    createPlayerElements() {
        ...
        this.players.forEach((player, playerId) => {
            ...
            // Add player color/styling based on playerId
    ...
    updateLocalPlayerPosition() {
        ...
        if (dx !== 0 && this.isColliding(localPlayer.position.x + dx, localPlayer.position.y)) {
            const gridY = Math.floor((localPlayer.position.y + GameConstants.TILE_SIZE / 2) / GameConstants.TILE_SIZE);
            const laneCenterY = gridY * GameConstants.TILE_SIZE;

            if (Math.abs(localPlayer.position.y - laneCenterY) <= GameConstants.CORNER_HELP_RANGE) {
        ...
        if (dy !== 0 && this.isColliding(localPlayer.position.x, localPlayer.position.y + dy)) {
            const gridX = Math.floor((localPlayer.position.x + GameConstants.TILE_SIZE / 2) / GameConstants.TILE_SIZE);
            const laneCenterX = gridX * GameConstants.TILE_SIZE;

            if (Math.abs(localPlayer.position.x - laneCenterX) <= GameConstants.CORNER_HELP_RANGE) {
    ...
    updatePassThroughBombs(player) {
        ...
        const boxRight = player.position.x + GameConstants.TILE_SIZE - 1 - GameConstants.COLLISION_GRACE;
        const boxTop = player.position.y + GameConstants.COLLISION_GRACE;
        const boxBottom = player.position.y + GameConstants.TILE_SIZE - 1 - GameConstants.COLLISION_GRACE;
    ...
    handlePowerupCollected(playerId, position, powerupType) {
        ...
        if (player) {
            ...
            const cellElement = document.querySelector(`[data-x="${position.x}"][data-y="${position.y}"]`);
            if (cellElement) {
                cellElement.className = 'cell empty';
            }
